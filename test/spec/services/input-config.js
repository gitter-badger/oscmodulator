describe('Service: inputConfig', function () {
  ...
  it('should default to one input.', function(){
    ...
    expect(inputConfig.inputs[0].id).not.toBeNull('The default input should have a valid id.');
  });
  ...
  it('should be able to add un-configured inputs.', function(){
    ...
    expect(inputConfig.inputs[0].id).not.toEqual(inputConfig.inputs[1].id, 'The ids should be distinct between inputs.');
  });

  it('should be able to add configured inputs.', function(){
    expect(inputConfig.inputs.length).toBe(1, 'By default there should be a single input.');

    inputConfig.addInput({id:20, name:'foo'});

    expect(inputConfig.inputs.length).toBe(2, 'A second input should be added.');
    expect(inputConfig.inputs[1].id).not.toEqual(20, 'The id should have been reset.');
  ...
  it('should be able to re-initialize the list of inputs.', function(){
    var list = [{id:10, name:'a'},{id:11, name:'b'},{id:12, name:'c'}];
    ...
    expect(inputConfig.inputs[0].id).not.toBe(10, 'The id of the first input should have been reset.');
    expect(inputConfig.inputs[1].name).toBe('b', 'The name of the second input should have been kept.');
    expect(inputConfig.inputs[2].id).not.toBe(12, 'The id of the last input should have been reset.');
  });
  ...
  it('should be able to create a copy of an existing input.', function(){
    ...
    expect(inputConfig.inputs[0].id).not.toEqual(inputConfig.inputs[1].id, 'The inputs should not have the same id.');
