...
angular.module('oscmodulatorApp').config(function($provide){
  $provide.factory('legato', function ($log, jq) {
		var legato = window.legato,
			utils = window.legatoUtils;
		...
		legato.midi = {
			...
			In:function(){
				...
				return function(messageRouter){
					// TODO expose this to a service that can be passed to the fake midi console
					window.router = messageRouter;
					return function(){
		...
		legato.receiveMidi = function(path, value){
			window.router(path, value);
		};
